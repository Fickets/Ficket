        ...
        <div className="flex flex-col items-center  bg-gray-400 min-h-screen">
            ...
            <div className="flex mt-[50px] mx-[10px] w-[300px] bg-white border border-[#666666]">
                ...
                <div className="flex flex-col w-3/5">
                    <p className="ml-[5px] mt-[5px] text-[25px]">{socketMessage?.data.ticket_id || "N/A"}</p>
                    <p className="ml-[5px] mt-[5px] text-[25px]">{socketMessage?.name || "N/A"}</p>
                    <p className="ml-[5px] mt-[5px] text-[25px]">{socketMessage?.birth || "N/A"}</p>
                    <p className="ml-[5px] mt-[5px] text-[25px]">{socketMessage?.data.similarity || "N/A"}</p>
                    <p className="ml-[5px] mt-[5px] text-[25px]">{socketMessage?.seatLoc || "N/A"}</p>
                </div>
            </div>

            {/* 버튼 섹션 */}
            <div className="my-[40px] bg-gray-400 py-5 w-full flex items-center justify-center">
                <button className="border border-white text-[30px] w-[100px] h-[50px] text-white bg-gray-800 hover:bg-gray-700"
                    onClick={checkButton}>
                    확인
                </button>
            </div>
